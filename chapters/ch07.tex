...
Chapter~\ref{ch:what-changes} ended with a verdict: a 7B model's training state needs roughly 140\,GB---too large for consumer GPUs and common 80\,GB training cards, and tight even on the largest single-GPU systems once activations and framework overhead are included. Distributed training is therefore the normal path for serious 7B training runs. But ``distributed training'' is not a single technique. It is a family of strategies, each answering a different question about what to split, what to replicate, and what price to pay in communication.
...
Chapter~\ref{ch:what-changes} did the arithmetic: a 7B model in mixed-precision training requires roughly 14\,GB for parameters (bf16), 14\,GB for gradients (bf16), 56\,GB for Adam optimizer states (fp32), and tens of GB for activations---a total well exceeding 100\,GB. This already exceeds common 80\,GB datacenter GPUs; newer 140--288\,GB cards change the boundary, but leave little practical headroom for long contexts, larger batches, or overhead. The question for most real training setups is not \emph{whether} to distribute, but \emph{how}.
...
\paragraph{What this chapter established.} Chapter~\ref{ch:what-changes} showed that a 7B model's training state pushes past ordinary single-GPU training budgets and quickly runs out of headroom once activations, context length, and optimizer state are included. This chapter provided the engineering toolkit for that regime: data parallelism for throughput, ZeRO/FSDP for memory, tensor and pipeline parallelism for extreme scale, activation recomputation for activation memory, and distributed checkpointing for reliability.
