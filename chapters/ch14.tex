...
\begin{enumerate}
  ...
  \item Explain why PPO uses a clipped surrogate objective instead of the raw policy gradient, and why clipping is applied per token.
  \item Describe the role of the value model (critic) in advantage estimation, and explain why eliminating it motivates critic-free algorithms like GRPO.
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Discount factor $\gamma \in [0, 1]$}: how much the agent values future rewards relative to immediate ones. Classical RL typically uses $\gamma < 1$ to ensure bounded returns in infinite-horizon problems. In LLM RL, episodes are finite (bounded by maximum sequence length), so $\gamma = 1$ (no discounting) is standard---the agent values all future tokens equally.
\end{enumerate}
...
\midrule
State $s_t$ & Sensor readings, positions & Prompt $+$ generated tokens \\
Action $a_t$ & Joint torques, button press & Next token from vocabulary $\mathcal{V}$ \\
Transition $P(s_{t+1} \mid s_t, a_t)$ & Stochastic physics & Deterministic append: $s_{t+1} = s_t \oplus a_t$ \\
Reward $r(s_t, a_t)$ & Dense per-step signal & Sparse: $r = 0$ until end-of-sequence \\
Episode length & 100--10{,}000 steps & 50--10{,}000+ tokens; reasoning models can be much longer \\
\bottomrule
\end{tabularx}
\normalsize
\end{center}
...
\paragraph{Densifying the sparse signal.} Sparse end-of-sequence reward makes credit assignment hard: which of 200 tokens caused the reward? One approach is \textbf{reward shaping}---adding intermediate rewards that guide the policy without waiting for the final score. Examples include format rewards (small bonus for well-structured output), length penalties (discouraging unnecessary verbosity), and \textbf{process reward models} (PRMs) that score individual reasoning steps rather than the final answer. Process rewards are central to reasoning RL; Chapter~\ref{ch:reasoning-rl} develops them in full. For now, the key insight is that sparse reward is the default in LLM RL, but it is not the only option---and much of the engineering of reasoning-RL systems involves finding principled ways to densify the signal.
...
\begin{itemize}[nosep,leftmargin=1.5em]
  ...
  \item Sparse reward is the default but not the only option: reward shaping and process rewards can densify the signal (Chapter~\ref{ch:reasoning-rl}).
\end{itemize}
...
Even small per-token changes compound. A per-token ratio of 1.05 means the new policy assigns 5\% more probability to that token---barely noticeable for any single token. But for a 200-token response: $1.05^{200} \approx 17{,}000$. This one sample would dominate a batch of thousands. This is why naively applying importance sampling to LLMs fails catastrophically---and why PPO (\S\ref{sec:ppo}) clips \emph{per-token} ratios rather than using the sequence-level ratio, bounding each $\rho_t$ to $[1{-}\epsilon, 1{+}\epsilon]$ so that the compounding cannot start.

\noindent\fbox{\parbox{\dimexpr\textwidth-2\fboxsep-2\fboxrule\relax}{%
\textbf{Pause and verify.} Your 3B model generates a 100-token response. After one gradient step, the per-token probability ratio averages $\rho_t = 1.02$. What is the approximate sequence-level ratio? ($1.02^{100} \approx 7.2$.) Now suppose the average per-token ratio is $1.10$. ($1.10^{100} \approx 1.4 \times 10^{4}$.) \emph{A 10\% per-token shift---barely visible at any single position---produces a sequence-level ratio in the tens of thousands. This is why per-token clipping is essential.}
}}
...
The advantage estimates in PPO require a value function $V(s_t)$---but where does it come from? In practice, the \textbf{value model} (also called the \textbf{critic}) is a separate neural network that learns to predict the expected return from any partial sequence. Its architecture is typically the same transformer backbone as the policy, with a single scalar output head replacing the vocabulary projection. It is often initialized from the SFT checkpoint or the reward model.

The value model is trained with a simple supervised objective: minimize the squared error between its prediction and the realized return:

\begin{equation}
\label{eq:value-loss}
\mathcal{L}_V = \mathbb{E}\!\left[\left(V_\phi(s_t) - R_t\right)^2\right]
\end{equation}

where $R_t$ is the Monte Carlo return from step $t$ (or a bootstrapped target). This loss is computed alongside the PPO policy loss at each training step---the value model improves its predictions as the policy changes.

\paragraph{The cost of a critic.} The value model is roughly the same size as the policy. Combined with the policy itself, the frozen reference policy, and the reward model, PPO-based RLHF requires \textbf{four model-sized objects} in memory simultaneously---this is the operational burden that Chapter~\ref{ch:preference-learning} \S\ref{sec:rlhf-motivation} described and that DPO was designed to avoid. It is also the key motivation for GRPO (Chapter~\ref{ch:reasoning-rl}), which eliminates the value model entirely by replacing $V(s_t)$ with a group-level mean reward computed from multiple samples of the same prompt. No critic, no critic-sized memory cost.

\subsection{Generalized Advantage Estimation (GAE)}

The advantage $\hat{A}_t$ in Equation~\ref{eq:ppo-clip} needs to be estimated. Given the value model from \S\ref{sec:value-model}, \textbf{Generalized Advantage Estimation} (GAE)~\cite{schulman2016gae} computes the advantage as an exponentially weighted average of $n$-step returns:
...
The quantity $\delta_t$ is the \textbf{TD residual}---the Bellman backup error from Equation~\ref{eq:bellman}. It measures how much the value function's prediction at $s_t$ mismatches the realized reward plus the discounted next-state value. If the value model were perfect, every $\delta_t$ would be zero. In practice, nonzero residuals tell the advantage estimator where the value model is surprised---and therefore where the policy update carries the most signal.

The parameter $\lambda \in [0, 1]$ trades off bias and variance: $\lambda = 0$ gives the one-step TD residual (low variance, high bias), $\lambda = 1$ gives the full Monte Carlo return (high variance, low bias). In practice, $\lambda = 0.95$ is the standard default for PPO-based RLHF.

\paragraph{GAE for LLMs.} In the standard LLM-RL setup with end-of-sequence reward only, intermediate rewards $r_t$ are zero for all $t < T$. GAE simplifies: the TD residual at each intermediate step reduces to $\delta_t = \gamma V(s_{t+1}) - V(s_t)$, capturing only the value model's evolving prediction. At the final step, $\delta_T = r_T - V(s_T)$, the difference between the actual reward and the value model's prediction. The value model thus serves as the learned baseline that §\ref{sec:policy-gradient} motivated: it tells the policy how good a ``typical'' response would be from any point in the generation, so that the advantage reflects how much \emph{better or worse} the actual continuation was.
...
\paragraph{Token-level clipping.} A subtle but important point: in practice, the PPO clipped objective is applied \emph{per token}, not per sequence. The importance ratio $\rho_t = \pi_\theta(y_t \mid x, y_{<t}) / \pi_{\theta_{\text{old}}}(y_t \mid x, y_{<t})$ is computed at each position, and clipping is applied to each $\rho_t$ individually. This is precisely the mechanism that defuses \S\ref{sec:importance-sampling}'s compounding-ratio problem: although the \emph{sequence-level} ratio can explode ($1.05^{200} \approx 17{,}000$), each \emph{per-token} ratio is bounded to $[1{-}\epsilon, 1{+}\epsilon]$, so per-token gradient updates remain stable. When you read RLHF or GRPO implementations, you will see the loss summed over tokens within each response, not computed once per response.

\paragraph{The full PPO objective.} Production PPO implementations add two further terms beyond the clipped surrogate:
\[
\mathcal{L} = \mathcal{L}^{\text{PPO}} + c_1 \cdot \mathcal{L}_V - c_2 \cdot \mathcal{H}[\pi_\theta]
\]
The value-function loss $\mathcal{L}_V$ (Equation~\ref{eq:value-loss}) trains the critic alongside the policy. The entropy bonus $\mathcal{H}[\pi_\theta]$ (with small coefficient $c_2 \approx 0.01$) prevents premature mode collapse by penalizing overly peaked token distributions. The entropy bonus is distinct from the KL penalty (\S\ref{sec:kl-rl}): KL measures distance from the \emph{reference policy}, entropy measures the policy's own distributional breadth. We focus on the clipped surrogate as the conceptual core, but you will encounter all three terms in code.
...
\begin{tcolorbox}[colback=blue!5, colframe=blue!40, title=\textbf{Worked Example: One PPO Step on Math}]
Walk through a single PPO training step for your 3B model with a math verifier.

\textbf{1.~Rollout.} Prompt: ``Solve: $7 \times 8 + 3$.'' The model generates 4 responses:
\begin{itemize}[nosep]
  \item $y_1$: ``$7 \times 8 = 56$, $56 + 3 = 59$. The answer is \textbf{59}.'' (32 tokens)
  \item $y_2$: ``Let me think... $7 \times 8 = 54$, so the answer is \textbf{57}.'' (28 tokens)
  \item $y_3$: ``$7 \times 8 + 3 = 59$.'' (12 tokens)
  \item $y_4$: ``The answer is $7 + 8 \times 3 = 31$. So \textbf{31}.'' (22 tokens)
\end{itemize}

\textbf{2.~Score.} The verifier checks final answers: $r = [1, 0, 1, 0]$.

\textbf{3.~Advantage.} The value model estimates $V(x) \approx 0.45$ for this prompt. Advantages: $\hat{A} = [+0.55, -0.45, +0.55, -0.45]$. Correct responses get positive advantage; incorrect ones get negative.

\textbf{4.~Per-token clipping.} Consider token $t{=}5$ in $y_1$, where $\pi_\theta$ assigns probability 0.12 and $\pi_{\theta_\text{old}}$ assigned 0.10. The per-token ratio is $\rho_5 = 1.2$. With $\epsilon = 0.2$, $\text{clip}(\rho_5, 0.8, 1.2) = 1.2$. The token is at the boundary---further probability increases for this token will produce no additional gradient signal.

\textbf{5.~Update.} Aggregate the per-token clipped objectives across all 4 responses. Take $K{=}4$ mini-batch gradient steps. Set $\pi_{\theta_\text{old}} \leftarrow \pi_\theta$. Sample new prompts and repeat.

\textbf{After this step}: the model is slightly more likely to produce response styles like $y_1$ and $y_3$ (correct), and slightly less likely to produce $y_2$'s arithmetic error or $y_4$'s operator-precedence mistake. Over thousands of steps, these small shifts compound into reliable math capability.
\end{tcolorbox}
...
\paragraph{A note on $\beta$.} The $\beta$ in Equation~\ref{eq:kl-rl-objective} and the $\beta$ in the DPO loss (Chapter~\ref{ch:preference-learning}, Equation~\ref{eq:dpo-loss}) play similar conceptual roles---both control how far the policy can drift from the reference---but they are not numerically comparable. DPO's $\beta$ scales the log-probability margin inside a sigmoid; RLHF's $\beta$ scales a KL divergence term against the reward. A DPO $\beta$ of 0.1 and a PPO $\beta$ of 0.05 are not directly comparable values. When you move between the two frameworks, do not transfer $\beta$ values.
...
Some implementations use \textbf{adaptive KL}: set a target KL divergence (e.g., 6 nats), and adjust $\beta$ dynamically to keep $D_{\text{KL}}[\pi_\theta \| \pi_{\text{ref}}]$ near this target. The rule is simple: when the measured KL exceeds $1.5\times$ the target, double $\beta$; when it falls below $0.5\times$ the target, halve $\beta$. This keeps the policy in a stable KL band without manual tuning. InstructGPT used this adaptive scheme~\cite{ouyang2022instructgpt}.
...
\caption{\textbf{The RL landscape: what we covered vs.\ what exists.} This chapter covered the policy-gradient family (REINFORCE $\to$ TRPO $\to$ PPO), which is the branch most directly used in LLM RLHF and reasoning-RL systems. The muted branches show representative areas we did not cover in depth---value-based methods, model-based methods, exploration-heavy methods, and multi-agent RL. The text also briefly positions offline RL and temporal credit assignment, but the figure is a scope map rather than a complete RL taxonomy.}
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Offline RL.} Learning from a fixed dataset without further environment interaction. DPO (Chapter~\ref{ch:preference-learning}) can be viewed as a form of offline RL: it optimizes a policy using a fixed preference dataset without rollouts from the current policy. The cost is what Chapter~\ref{ch:preference-learning} \S\ref{sec:iterative-dpo} called distribution shift---the training pairs may not reflect the current policy's behavior. Online DPO partially addresses this by reintroducing rollouts, which operationally brings it closer to the RL loop described in this chapter.
...
\begin{itemize}[nosep,leftmargin=1.5em]
  ...
  \item We omitted value-based methods, model-based RL, exploration theory, multi-agent RL, offline RL in depth, and temporal credit assignment in detail because they are not central to the post-training methods in Chapters~\ref{ch:rlhf} and~\ref{ch:reasoning-rl}.
...
\begin{enumerate}
  ...
  \item PPO clips the importance sampling ratio \emph{per token} to prevent large policy updates, achieving trust-region-like stability with simple first-order optimization. The per-token clipping defuses the compounding-ratio problem that makes sequence-level importance sampling impractical for long responses.
  \item The value model (critic) learns to predict expected return from any partial sequence. It is roughly policy-sized, making it a significant memory cost. GRPO (Chapter~\ref{ch:reasoning-rl}) eliminates it with a group-level baseline.
  \item The KL penalty to a reference policy prevents reward hacking, preserves capability, and maintains output diversity. It is the same objective that underlies DPO (Chapter~\ref{ch:preference-learning}), but the $\beta$ values are not numerically comparable between DPO and PPO.
\end{enumerate}
\end{takeaways}
