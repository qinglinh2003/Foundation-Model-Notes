...
\subsection{From MHA to GQA: Shrinking the Cache}
\label{sec:gqa}

The 2.1\,GB number above assumes standard multi-head attention (MHA), where each of the $h$ query heads has its own K and V projections---all $h$ sets of K/V vectors must be cached. \textbf{Grouped Query Attention (GQA)} addresses this by sharing K/V heads across groups of query heads. If MHA uses $h = 32$ query heads with $h = 32$ KV heads, GQA might use 32 query heads but only $n_{\text{kv}} = 8$ KV heads. Each group of 4 query heads shares the same K and V projections.
...
The tradeoff:
\begin{itemize}[nosep]
  \item \textbf{KV cache shrinks by $h / n_{\text{kv}}$.} With $n_{\text{kv}} = 8$ instead of 32, cache drops from 2.1\,GB to roughly $2.1 / 4 \approx 0.5$\,GB. That is the difference between serving 4 users and serving 16 on the same hardware.
...
GQA's value is not in training quality; it is in deployment economics. This is why every major open-weight model released since 2023---LLaMA-2 70B, Mistral, Qwen---uses GQA by default.
...
\begin{itemize}[nosep,leftmargin=1.5em]
  ...
  \item KV cache size = $2 \times L \times n_\text{kv} \times T \times d_k$ elements. For a 7B model at 4K context: $\sim$2\,GB (MHA).
  \item GQA shares K/V heads across query groups, shrinking the cache by $h / n_\text{kv}$ (e.g., $4\times$ with $n_\text{kv} = 8$).
