"""Ablation 2: Depth vs Width at ~3.7M params."""
import os, json, torch, time, math, sys
...
configs = [
    {"name": "deep_narrow",   "n_layers": 8, "d_model": 192, "n_heads": 4, "d_ff": 768},
    {"name": "baseline",      "n_layers": 4, "d_model": 256, "n_heads": 4, "d_ff": 1024},
    {"name": "shallow_wide",  "n_layers": 2, "d_model": 384, "n_heads": 4, "d_ff": 1536},
]
...
for cfg in configs:
    ...
    sep = "=" * 60
    print(f"\n{sep}")
    print(f"ABLATION 2: {name} (L={cfg['n_layers']}, d={cfg['d_model']})")
    print(sep)
    ...
    run = wandb.init(project="capstone-project1-minigpt", name=f"abl2_depth_{name}", reinit=True)
    ...
    while step < 10000:
        ...
        if step % 200 == 0:
            ...
            wandb.log(entry)
            if step % 2000 == 0:
                print(f"  step {step}: val={val_loss:.4f}")
            model.train()
    ...
    result = {"name": name, "n_layers": cfg["n_layers"], "d_model": cfg["d_model"],
              "params": params, "val_loss": final_val, "bits_per_byte": bits_per_byte, "wall_time": elapsed}
    all_results.append(result)
    print(f"  DONE: val={final_val:.4f}, bpb={bits_per_byte:.4f}, params={params:,}, time={elapsed:.0f}s")

    result_dir = f"results/abl2_depth/{name}"
...
os.makedirs("results/abl2_depth", exist_ok=True)
with open("results/abl2_depth/summary.json", "w") as f:
    json.dump(all_results, f, indent=2)

print("\n\nABLATION 2 SUMMARY")
for r in sorted(all_results, key=lambda x: x["val_loss"]):
    print(f"  {r['name']:20s}: L={r['n_layers']}, d={r['d_model']}, params={r['params']:,}, val={r['val_loss']:.4f}, bpb={r['bits_per_byte']:.4f}")
